...
<body>
							...
							$scope.search = function() {
								$http
										.get(
											...
											$scope.names = response.data;
											alert('$scope.names='+$scope.names);
										});
							};
							
							/*XSL action - (START)*/
							$scope.xslEvent = function() {
								/* $http.get("http://localhost:8080/StrutsDisplayTag/xsltAction.do")
										.then(function(response) {
											//$scope.names = response.data;
											//templateUrl: '/xslt.jsp';
											var absUrl = $location.absUrl();
											alert('got response2='+absUrl);
										}); */
										//http://52.52.200.217:8082/StrutsDisplayTag/xsltAction.do";
								$window.location.href = "http://localhost:8080/StrutsDisplayTag/xsltAction.do";
							};
							/*XSL action - (END)*/
						});
	...
	<div ng-app="myApp" ng-controller="customersCtrl">
		...
		<div id="main"
			style="width: 100%; height: 100%; padding: 50px; font-weight: bold;display: none;" >
			<div id="fetchDataDiv" align="center" >
				<button class="fetchbutton1" type="submit" ng-click="load()"
		...
		<div id="editPopup"
			style="height: 100%; top: 0; left: 0; position: fixed; opacity: 1; filter: alpha(opacity =       100); display: none; z-index: 2;display: none;"
			align="center">
			<div id="popup"
		...
		<div id="xslId">
			<center><div>Click the button to see the XSLT response</div>
			<input type="submit" class="fetchbutton" value="xsl response" ng-click="xslEvent()"/></center>
